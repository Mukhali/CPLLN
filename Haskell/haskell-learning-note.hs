...
-- 键入 `ghci` 开始 repl。（在 ghci 和一般的文件中操作有些不一样）
-- 现在你可以键入 Haskell 代码。
-- 任何新值都需要通过 `let` 来创建：
...
[1..]    -- 一个含有所有自然数的列表（在 REPL 环境下输入会产生无限循环）
...
-- 元组中的每一个元素可以是不同类型的，但是一个元组的长度是固定的
("haskell", 1)
...
-- 并且 `xs` 是列表剩余的部分。我们可以写自己的 map 函数
myMap func [] = []
myMap func (x:xs) = func x:(myMap func xs)

-- 编写出来的匿名函数带有一个反斜杠，后面跟着所有的参数。
myMap (\x -> x + 2) [1..5] -- [3, 4, 5, 6, 7]

-- 使用 fold （在一些语言称为`inject`）随着一个匿名的函数。
-- foldl1 意味着左折叠(fold left), 并且使用列表中第一个值
-- 作为累加器的初始化值。
...
-- Haskell 有一个非常强壮的类型系统，一切都有一个类型声明。
...
-- 任何与外界交互的函数(也就是 IO) 都在它的类型声明处做一个 `IO` 标志
