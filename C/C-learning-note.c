...
#include "my_header.h"  //用户自己定义的

// 函数声明可以在.h文件或者在.c文件的头部定义
...
// main函数需要返回值，可以不给，建议有
int main() {

// printf输出到标准输出，可以设定格式，%d 代表整数, \n 代表换行
printf("%d\n", 0); 
// 所有的语句都要以分号结束
...
// C 语言中变量必须先声明，并指定类型，这样系统可以决定这个变量的空间
...
// long型（长整型）一般需要4个字节到8个字节
// 而long long型则至少需要8个字节（64位）
long x_long = 0;
...
// 字符串就是以 NUL (0x00)或0 这个字符结尾的字符数组,
...
// C中没有布尔类型 而是用整形替代，(C99中有_Bool或bool。)
...
// 使用<limits.h>提供的CHAR_MAX、SCHAR_MAX和UCHAR_MAX宏可以确定`char`、
// `signed_char`和`unisigned char`的最大值。
...
// 指针变量是用来储存内存地址的变量，指针变量的声明指明数据的类型
...
// &arr的类型不是`int *`！它的类型是指向数组的指针（数组由10个int组成）
...
// 字符串通常是字符数组，用字符指针表示(它是指向数组的第一个元素的指针)
...
/*
只要函数声明是正确的，任何时候都能将任何函数赋给某个函数指针
